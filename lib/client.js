...
class Client {
  constructor(logger, cfg) {
    this.logger = logger;
    this.cfg = cfg;
    this.cfg.conflictBehaviour = cfg.conflictBehaviour|| 'fail';
  ...
  async createFolder(id, name) {
    const response = await this.restClient.makeRequest({
      url: `/drives/${this.cfg.driveId}/${id}/children`,
      method: 'POST',
      body: {
        name,
        folder: { },
        '@microsoft.graph.conflictBehavior': this.cfg.conflictBehaviour,
      },
    });
