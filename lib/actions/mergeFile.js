...
exports.process = async function (msg, cfg) {
  ...
  const attachment = Object.keys(msg.attachments)[0].url;
  ...
  const exist = await client.isExist(path);
  if (exist) {
    this.logger.trace('Replacing file that already exist in path: %s', path);
    const { body } = await client.replaceFile(path, content);
    this.logger.trace('Successfully replaced file in path: %s', path);
